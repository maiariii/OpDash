...
const MultiSelect = ({ label, options, selected, onChange, placeholder = "Select...", className = "" }) => {
    ...
    return (
        <div className={`relative ${className}`} ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute z-50 mt-1 w-full min-w-[220px] bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                    {/* Select All Option */}
                    <div
                        className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-slate-100 flex items-center border-b border-slate-100"
                        onClick={handleSelectAll}
                    >
                        <div className={`flex items-center justify-center w-4 h-4 mr-2 border rounded ${selected.length === options.length ? 'bg-blue-600 border-blue-600' : 'border-slate-300'}`}>
                    ...
                    {options.map((option) => {
                        ...
                        return (
                            <div
                                key={option.value}
                                className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-slate-100 flex items-center"
                                onClick={() => toggleOption(option.value)}
                            >
                                <div className={`flex items-center justify-center w-4 h-4 mr-2 border rounded ${isSelected ? 'bg-blue-600 border-blue-600' : 'border-slate-300'}`}>
                                    {isSelected && <Check className="h-3 w-3 text-white" />}
                                </div>
                                <span className={`block truncate ${isSelected ? 'font-medium text-slate-900' : 'font-normal text-slate-600'}`}>
                                    {option.label}
                                </span>
                            </div>
                        );
